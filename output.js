...
async function processAccount(account, index) {
  ...
  try {
    ...
    await Promise.all([corsToActivity(memberID, uid, phone, nickName, index), saveUserDeviceInfo(memberID, uid, phone, index)]);
    ...
    if (cachedData?.token) {
      try {
        const signResponse = await retryTask(async () => {
          return await pkPost("/ehomes-new/pkHome/api/bonus/signActivity2nd", {
